<header id="header">
    <nav class="navbar is-transparent">
        ...
        <div id="navbarExampleTransparentExample" class="navbar-menu">
            <a v-on:click.prevent="go_to('carousel', false, $event)" :class="logo_class()" href="/">
                <% if $SiteConfig.SiteLogo %>$SiteConfig.SiteLogo.SetHeight(60)<% else %>$SiteConfig.Title<% end_if %>
            </a>
            <a v-on:click.prevent="go_to(navitem.scrollto, navitem.has_hero, $event)" v-for="(navitem, index) in navitems" :class="make_class(index)" :href="navitem.url">{{navitem.title}}</a>
