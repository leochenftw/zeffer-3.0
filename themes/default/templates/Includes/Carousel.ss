<div id="carousel" class="owl-carousel">
    <div v-for="item in carousel" :class="{'caro-slide': true, 'text-heavy': !item.title_as_link, 'has-text-centered': item.title_as_link}">
        ...
        <h2 class="container caro-slide__content title is-1" v-else-if="!item.overlay">
        ...
        <a class="btn-go-to" href="#" :data-scrollto="item.scroll_to"><img v-if="item.overlay" class="caro-slide__overlay" :src="item.overlay" :alt="item.title" /></a>
    </div>
</div>
